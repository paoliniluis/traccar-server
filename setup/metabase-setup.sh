...
# creating a postgres
curl -s -X POST http://$1/api/database -H 'Content-Type: application/json' --cookie "metabase.SESSION=$sessionToken" --data '{"engine":"postgres","name":"pg","details":{"host":"timescaledb","port":"5432","dbname":"gps","user":"postgres","password":"password","schema-filters-type":"inclusion","schema-filters-patterns":"public","ssl":false,"tunnel-enabled":false,"advanced-options":false},"is_full_sync":true}' &&
